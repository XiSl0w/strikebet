<script>
  ...
  import Upcoming from "$lib/components/upcoming_mod.svelte";
  ...
  $: display_live = live_matches.filter(
    (match) =>
      match.league.name === selected_league || selected_league === "All"
  );
  $: display_upcoming = upcoming_matches.filter(
    (match) =>
      match.league.name === selected_league || selected_league === "All"
  );
  ...
  async function get_upcoming_data() {
    ...
    let response = await fetch(
      `http://127.0.0.1:3220/bets/pre-match/odds?date=${date}`,
      {
        method: "GET",
        headers: AuthActions.getAuthHeader(),
      }
    ...
    if (response.ok) {
      ...
      leagues = temp.filter(
        (league, index, self) =>
          self.findIndex((t) => t.name === league.name) === index
      );
  ...
  <div class="live">
    ...
    <div class="live-games" use:ref>
      ...
      {:else if display_live.length == 0}
        <div class="bets-loading">No live matches available</div>
      {/if}
